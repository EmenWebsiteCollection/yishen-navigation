...
import { getWorkById, updateWork, listGroups, WORK_TYPES, WORK_STATUS, isAdmin, CREATIVE_TYPES, AI_DEGREES, AUDIENCES, CONTENT_WARNINGS, workTypeLabel, sanitizeHttpUrl, hasLinkType } from '../services/works.js';
...
'  };'
  const handleSubmit = async (e) => {
    ...
    if (hasLinkType(workTypes.join(','))) {
      if (!url.trim()) {
        setError('链接类作品（网站/游戏/音乐/视频）必须填写 URL');
        return;
      }
    ...
    <div className="ym-detail-layout">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: '16px' }}>
          <label htmlFor="edit-url" style={labelStyle}>{hasLinkType(workTypes.join(',')) ? 'URL' : 'URL（可选）'}</label>
          <input
            id="edit-url"
            type="url"
            value={url}
