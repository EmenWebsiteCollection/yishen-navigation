...
- 账号不存在时**也返回发送成功**，防止通过接口枚举已注册邮箱；request 分支所有出口（未注册 / 触发限频 / 发送失败）统一返回同一成功响应，并垫时延到与真实发信重叠的 3-5 秒窗口，状态码、文案、时长均不泄露注册状态（#139）。
...
| 60 秒内重复请求验证码 | 触发了 60 秒限频（接口仍返回成功，不重发也不提示） | 防枚举设计：正常等待即可 |
| 收不到邮件但接口返回成功 | 进了垃圾箱 / 该邮箱未绑定任何账号 / 发信服务临时故障（对外统一报成功） | 查垃圾箱；确认已在个人中心绑定 |
