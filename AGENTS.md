...
- Mostly-client SPA: the browser talks directly to Supabase via the anon-key JS client in `src/services/`. **A few Netlify Functions do exist** in `netlify/functions/` (`screenshot.mjs`, `password-reset.mjs`, plus `el-memory.mjs`/`yili-chat.mjs`) — they are Functions API v2 (`export default async (req) => Response`). They exist specifically for work that needs secrets the browser must never hold (`SUPABASE_SERVICE_ROLE_KEY`, mail-provider keys). Everything else still goes straight to Supabase. `docs/tech.md` describes an aspirational "all data via Netlify Functions" design the code does not implement; trust the code.
...
- **Secrets policy (#147)**: every `VITE_*` env var is bundled into the public JS and must be treated as public information — never put real secrets there. Server-only secrets (`SUPABASE_SERVICE_ROLE_KEY`, mail/LLM provider keys) live only in Netlify environment variables, never in `.env*` files that could be committed.
